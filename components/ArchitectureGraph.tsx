...
const LayerPill: React.FC<{ label: string; color: string; count: number }> = ({ label, color, count }) => (
  <div
    ...
    style={{ background: `${color}18`, border: `1px solid ${color}44`, color: 'var(--s-text-muted)' }}
  >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-base font-bold truncate" style={{ color: 'var(--s-text)' }}>{node.label}</p>
        ...
        <button
          ...
          style={{ background: 'var(--s-hover-over)', color: 'var(--s-text-muted)' }}
          aria-label="Close node info"
        >✕</button>
      </div>
      <p className="mt-3 text-sm leading-relaxed" style={{ color: 'var(--s-text-muted)' }}>{node.desc}</p>
      <div className="mt-3 pt-3 flex flex-wrap gap-1.5" style={{ borderTop: '1px solid var(--s-border)' }}>
...
export const ArchitectureGraph: React.FC = () => {
  ...
  return (
    ...
    >
      {/* ─ Header ────────────────────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-5 pt-4 pb-3 shrink-0" style={{ borderBottom: '1px solid var(--s-border)' }}>
        <div>
          <h2 className="text-lg font-black tracking-tight" style={{ color: 'var(--s-text)' }}>
      ...
      >
        <p className="text-[9px] uppercase tracking-widest mb-2" style={{ color: 'var(--s-text-dim)' }}>Layer Legend</p>
